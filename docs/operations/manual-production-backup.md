...
(применение migration 0032) выполнено.** Само применение миграции
по-прежнему требует отдельного явного подтверждения владельца (не
выполняется автоматически этим документом) и не было выполнено в
рамках этой задачи.

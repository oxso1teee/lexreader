...
- 2026-08-05 — Applied the two approved migrations locally and wired `profiles.reader_settings` for real cross-device sync (extracted pure prefs logic into `reader-prefs.ts` so the Server Component could read it; `page.tsx` passes the account's saved settings to `Reader`, which adopts them once on mount and pushes changes back via a new `updateReaderSettings` action). Verified end-to-end via Playwright. Production application handed to the user (SQL + rollback provided) since this environment only holds local Supabase credentials.
- 2026-08-05 — Opened Draft PR #13, pushed, and iterated against real hosted CI (GitHub Actions), which turned out to be far more informative than local-only verification:
  - Discovered CI's `e2e` job had been failing on **every** recent `main` merge, unrelated to this Slice (`@supabase/supabase-js` now needs Node's native `WebSocket`, stable since Node 21, but the workflow pinned Node 20). Fixed in a separate PR (#14, `fix/ci-node-version`) and merged into this branch too so this PR's own CI could run at all.
  - Fixed a real test bug: `add-material.spec.ts`'s `getByRole("button", { name: "PDF" })` (no `exact: true`) matched both the PDF/Photo toggle and a hidden file input whose accessible name contains "PDF" as a substring.
  - Root-caused (via temporary console/network diagnostics added to `reading.spec.ts`, run through CI, then removed) a real, worthwhile bug: Library material cards had no `prefetch={false}`, so Next's default Link prefetching fired the **full** dynamic `/read|/watch/[id]` route (every DB query the Reader page makes — that route has no `loading.js` boundary to prefetch only a shell) for every visible card the instant the Library page mounted. Confirmed via CI log output: a burst of a dozen+ concurrent `_rsc=` prefetch requests to unrelated materials fired on every Library load. Fixed at the source (`library-item-card.tsx`) — a genuine improvement for real users too (wasted DB/bandwidth for cards never opened), not just a CI workaround.
  - **Residual, not fully resolved**: even after that fix, `reading.spec.ts`'s Library-click-to-Reader navigation assertion still failed intermittently in CI (~2 of 4 subsequent runs — sometimes at the navigation step, once at a later translation-popup step), while passing 100% reliably in every local reproduction (4/4 isolated runs and 1/1 full-suite run against a real `next build && next start` server with normal resource headroom). One CI rerun also hit a pure Docker port-binding failure before any test ran at all (`address already in use` starting `supabase_inbucket`), independently confirming this specific CI environment has real, code-unrelated infrastructure instability. Per the task's own instruction not to burn hours in an infinite e2e loop, stopped chasing this after ~7 total CI runs plus extensive local reproduction attempts. **Not treated as a Slice 3 regression**: the underlying feature was directly, manually verified working correctly in-browser throughout this whole Slice's development, `main`'s own CI had no reliable e2e signal for many merges before this Slice even started (making "compare against a working main CI baseline" impossible), and two real, legitimate bugs were found and fixed along the way regardless of whether they fully explain 100% of the remaining variance. Revisit condition: if this reproduces the same way on a subsequent, unrelated CI run (i.e., persists after this Slice merges), investigate as a standalone CI-infra ticket, not by re-opening this Slice.

...
Status: **Phase A–D implemented on `feature/missions-v1`, not yet merged.** Durable source of truth for Slice 6 — read this first if resuming in a new session, before re-deriving anything from chat history. Production already contains Slice 1 (App Shell + Today), Slice 2 (Progress + Settings), Slice 3 (Library + Reader), Slice 4 (Practice/Brain/Review), Slice 4.1 (accessibility cleanup), Slice 5 (Language Twin v1, merged as `f2bd582`). Migration `0037_missions.sql` has already been applied to the shared Preview/Production Supabase project (schema-only — no UI was live before this branch's Phase B–D work).
...
## 13–15. Integration (Language Twin / Practice / Today / Progress) — as actually implemented

- **Language Twin** (`/language-twin`): "Сейчас в фокусе" gains a second CTA, "Потренировать сейчас", next to the existing "Открыть паттерн →" — shown only when an active mission (`available`/`started`) already exists for that exact `source_pattern_id`; never fabricates one. (Simplification vs. the original sketch: no separate "Последняя практика: сегодня" timestamp line — the mission's own status/priority badges on the Details screen already carry that signal.)
- **Practice**: zero changes to Brain/Practice pages beyond one new optional `missionId` query param threaded alongside the existing `wordIds` param (same pattern) — missions link *into* `/brain/all/review?wordIds=...&missionId=...` and reuse `SessionComplete` (extended, not rewritten) + `getLanguageTwinUpdateAction` verbatim.
- **Today**: one new conditional section, same gating shape as `LanguageTwinSummaryCard` (`missions.length > 0`), showing up to 3 active missions via the shared `MissionCard` component (no separate "primary vs. compact" visual treatment — one consistent card size, ranked by the engine's own ordering). No Today v2, no layout rewrite.
- **Progress**: one new stat pair ("Миссий завершено за неделю" / "Направлений затронуто"), computed with the same `isoWeekStart()` shape as `getReviewsThisWeekCount`, shown only when ≥1 mission was completed this week (avoids a perpetual all-zero card for users who haven't touched Missions).
...
## 17. Analytics (privacy-safe) — as actually implemented

Events: `mission_impression`, `mission_opened`, `mission_started`, `mission_step_completed`, `mission_completed`, `mission_dismissed`, `mission_resumed`, `mission_result_viewed`. Payload is deliberately leaner than first sketched above: `mission_type` (the 9-value schema enum) plus, only on `mission_step_completed`, `step_index`/`correct`, and on `mission_impression`, `mission_count`. No `skill_category`/`difficulty`/`priority`/duration bucket, and — the one disclosed departure from the original sketch — **no raw `mission_id`**, following the same reasoning Language Twin already uses (`category`/`confidence` instead of a pattern id): an opaque per-row identifier is still a joinable handle back to that mission's own title/content, so `mission_type` is used everywhere instead. `mission_completed` fired from the targeted-practice path (`session-complete.tsx`) carries no properties at all, since that path never learns the mission's type. Never sent: sentence/word/phrase/translation/correction/material content/question prompt/options/explanation/free-form answer text — enforced by `e2e/missions-privacy.spec.ts`, a new regex-enforcement test mirroring `language-twin-privacy.spec.ts`, scanning every file that fires a `mission_`-prefixed event.

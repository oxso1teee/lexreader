...
(() => {
  const DOM_ONLY = location.hash.includes("lexreader-extraction-dom-only");
  ...
  // to us even though it reaches the page just fine.
    ...
    // Literal DOM-only proof mode: timedtext fetch/XHR observation is not
    // installed, so a network response cannot accidentally satisfy the run.
    console.debug("[LexReader:diag] DOM-only mode: network transcript capture disabled");
  }
  ...
  if (location.hash.includes("lexreader-extraction")) {
    ...
    const pauseIfPlaying = () => {
      ...
      if (!autoplayDisabled) {
        const autoplayToggle = document.querySelector(
          ".ytp-autonav-toggle-button[aria-checked='true'], button[aria-label='Autoplay is on'], button[aria-label*='Автовоспроизведение включено' i]",
        );
        if (autoplayToggle) {
    ...
    pauseIfPlaying();
    const guardInterval = setInterval(pauseIfPlaying, 100);
    // Bounded to cover background.mjs's single emergency ceiling -- never
    // guards a tab forever.
    setTimeout(() => clearInterval(guardInterval), 100_000);
  }
